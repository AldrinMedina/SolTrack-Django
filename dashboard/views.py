...
def download_contract_report(request, contract_id):
    ...
    buyer_org=contract.buyer.organization if contract.buyer else "N/A"
    ...
    seller_org=contract.seller.organization if contract.seller else "N/A"
    ...
    # Section 1.5: Itemized Bill
    content.append(Paragraph("💰 Itemized Bill", section_header))

    # Example: Pull product/item details — adjust based on your model
    # If you have a related model like ContractItem, you can replace this list with a query
    # e.g. items = ContractItem.objects.filter(contract=contract)
    # For demo, let's generate from contract.product_name and quantity
    
    item_data = [
        ['Description', 'Batch Size', 'Unit Price (ETH)', 'Subtotal (ETH)']
    ]

    # If contract has itemized details stored elsewhere, replace this block accordingly
    # For now, assume single product in contract
    unit_price = (contract.price / contract.quantity) if contract.quantity else contract.price
    ...
    item_data.append([
        '',
        '',
        Paragraph('<b>Total</b>', normal_text),
        Paragraph(f"<b>{total:.4f} ETH</b>", normal_text)
    ])
    # Create Itemized Table
    item_table = Table(item_data, colWidths=[3*inch, 1*inch, 1.3*inch, 1.4*inch])
    ...
    Verified and secured by blockchain technology on Ganache Test Network<br/>
...
#         contract.save()
#         return JsonResponse({'message': f'Contract {contract_id} marked as Completed.'})
#     except Contract.DoesNotExist:
#         return JsonResponse({'error': f'Contract {contract_id} does not exist.'}, status=404)
